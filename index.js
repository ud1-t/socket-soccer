...
function connected(socket) {
    socket.on('newPlayer', (data) => {
        ...
        serverBalls[socket.id] = new Capsule(data.x, data.y, data.x + 80, data.y, 40, 5)
        ...
        console.log(`Total players: ${Object.keys(playerPos).length}`)
        console.log('Players: \n', playerPos)
...
function serverLoop() {
    ...
    for(let i in serverBalls) {
        playerPos[i].x = serverBalls[i].pos.x
        playerPos[i].y = serverBalls[i].pos.y
        playerPos[i].angle = serverBalls[i].angle
    }
